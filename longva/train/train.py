...
class DataArguments:
    ...
    anet_video_folder: Optional[str] = field(default=None)
    didemo_video_folder: Optional[str] = field(default=None)
    internvid_video_folder: Optional[str] = field(default=None)
...
def preprocess(
    ...
    if conversation_lib.default_conversation.version == "qwen_1_5":
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        try:
            ...
            if 'image' in sources[0]:
                ...
                video_file = f"{self.list_data_dict[i]['video']}.mp4"
                from_source = self.list_data_dict[i].get('source', None)

                processor = self.data_args.image_processor
                if from_source is None:
                    video_folder = self.data_args.video_folder
                elif from_source == 'anet':
                    video_folder = self.data_args.anet_video_folder
                elif from_source == 'didemo':
                    video_folder = self.data_args.didemo_video_folder
                elif from_source == 'internvid':
                    video_folder = self.data_args.internvid_video_folder
